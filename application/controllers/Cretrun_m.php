...
class Cretrun_m extends CI_Controller {
    ...
    public function invoice_return_form() {
        ...
        $invid = $this->db->select('invoice_id')->from('invoice')->where('sales_by',$this->session->userdata('user_id'))->where('invoice', $invoice_id)->get()->row();
        $query = $this->db->select('invoice_id')->from('invoice')->where('sales_by',$this->session->userdata('user_id'))->where('invoice', $invoice_id)->get();
